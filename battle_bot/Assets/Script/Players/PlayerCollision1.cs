...
public class PlayerCollision1 : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.CompareTag("atwp2"))
        {
            // 체력 관리를 GameManager에서 가져와 사용
            int playerHealth = GameManager.instance.GetPlayerHealth(1);  // 플레이어 1의 체력을 가져옴
            playerHealth -= 10;  // 체력 감소
            GameManager.instance.SetPlayerHealth(1, playerHealth);  // 감소된 체력을 GameManager에 설정
